...
@lru_cache(maxsize=1)
def load_patients_csv() -> list[dict[str, str]]:
    """Load the denormalized patient table from data/patients/patients.csv.

    Generated by scripts/generate_patients_csv.py from the FHIR bundles.
    """
    path = config.PATIENTS_DIR / "patients.csv"
    if not path.exists():
        return []
    rows: list[dict[str, str]] = []
    with open(path, "r", encoding="utf-8", newline="") as f:
